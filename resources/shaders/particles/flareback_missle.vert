...
uniform float uMaxRadius_frac; // 最大尾焰半径与最小尾焰半径的比值
...
void main() {
    ...
    if (age > 0.0) {
        ...
        // - 径向飞出（dir * t * speed）
        // - 轻微上升（+ vec3(0, t*0.5, 0)）
        vec3 center = uExplosionPos + inEmitOffset * ( 1.0 * (1.0 - t) + t * uMaxRadius_frac) +  dir * t * scale ;
        ...
        vec3 baseColor;
        if (t < 0.8) {
            baseColor = mix(vec3(1.0, 0.9, 0.6), vec3(1.0, 1.0, 0.8), t / 0.8);
        } else {
            baseColor = mix(vec3(1.0, 1.0, 0.8), vec3(0.7, 0.7, 0.7), (t - 0.8) / 0.2);
        }

        float alpha = 1.0; // 粒子的透明度非线性衰减
