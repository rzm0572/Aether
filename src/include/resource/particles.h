...
class Particle_Fireball_explosioin {
public:    
    /**
     * @brief 构造函数，传入粒子数量，随机种子，贴图路径
     * @param particle_num 粒子数量
     ...
     * 具体设计思路如下
    */
    Particle_Fireball_explosioin(int particle_num,
                      unsigned int random_seed,
                        const std::string& texture_path){
        // 加载纹理
        int width, height, channels;
...
public:
    ...
    // 批量添加粒子
    void addParticles(const glm::vec3& emit_pos,const glm::vec3& obj_dir, int num_particles = 8,float delta_size = 0.2f) {
        srand(time(nullptr));
        ...
        for (float j = 0.0f; j < emit_dist; j += delta_size){
            for (int i = 0; i < num_particles; ++i) {
                ...
                particles[idx] = {
                    last_emit_pos + emit_dir * (j + delta_size * static_cast<float>(rand()%3 - 1) * 0.5f),
                    now,
                    offset
                };
...
public:    
    ...
    */
    Particle_Bullet(int particle_num,
        ...
        */
        ...
        for(int i=0;i<particle_num;i++){// 初始化粒子，对于每个粒子只要初始化一遍即可
            float Time_bias=static_cast<float>(rand()%10000)/10000.0f;
            // 在球面随机一个位置
            float V_x=(static_cast<float>(rand()%10000)/10000.0f)*static_cast<float>(rand()%3-1);
            float V_y=(static_cast<float>(rand()%10000)/10000.0f)*static_cast<float>(rand()%3-1);
            float V_z=(static_cast<float>(rand()%10000)/10000.0f)*static_cast<float>(rand()%3-1);
            // float V_len=glm::length(glm::vec3(V_x,V_y,V_z));
            // if(V_len>=0.000001f){
            //     V_x/=V_len;
            //     V_y/=V_len;
            ...
            // V_len = static_cast<float>(rand()%10000);
            // V_x*=V_len;
            // V_y*=V_len;
            // V_z*=V_len;
            // 四个顶点
        ...
        // 着色器
        if (!ServiceLocator<ShaderManager>::get()->isShaderRegistered("bullet")) {
            // 注册着色器
     ...
     * @param max_size 粒子最大尺寸，用于控制粒子大小
    */
    void draw(glm::vec3 World_pos,glm::vec3 Velocity, glm::mat4 view, glm::mat4 proj, glm::vec3 camera_pos,float scale=3.0f,float life_time = 3.0f,float max_size = 0.1f,float Button_size = 0.3f){
        // 在绘制粒子前开启混合
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // 标准透明混合
        float time_now = glfwGetTime();
        ...
        if(is_exploding){
            ...
            shader->setUniform("life_time", life_time);// 设定粒子能飞多长时间
            shader->setUniform("Max_size", max_size);// 设定粒子最大尺寸
            shader->setUniform("uCenterVelocity",Velocity );
            shader->setUniform("Button_size", Button_size);
    ...
    */
    Tergeo(int particle_num,
        ...
        for(int i=0;i<particle_num;i++){// 初始化粒子，对于每个粒子只要初始化一遍即可
            ...
            float V_z=(static_cast<float>(rand()%10000)/10000.0f)*static_cast<float>(rand()%3-1);
            float V_x=glm::sin(2.0f*V_z*num_of_rounds*glm::pi<float>());
            float V_y=glm::cos(2.0f*V_z*num_of_rounds*glm::pi<float>());
            // 四个顶点
            for(int j=0;j<4;j++){
                // 第一位是粒子序号，粒子序号暂时用于随机种子，用随机算法标识粒子的发射方向
                vertices.push_back(static_cast<float>(i));
        ...
        // 着色器
        if (!ServiceLocator<ShaderManager>::get()->isShaderRegistered("magic")) {
            // 注册着色器
            ServiceLocator<ShaderManager>::get()->registerShader("magic", getShaderPath("particles/magic.vert"), getShaderPath("particles/magic.frag"));
        }
        // 解绑
        glBindVertexArray(0);
    }
     ...
     * @param max_size 粒子最大尺寸，用于控制粒子大小
    */
    void draw(glm::vec3 World_pos,glm::vec3 Velocity, glm::mat4 view, glm::mat4 proj, glm::vec3 camera_pos,float scale=3.0f,float life_time = 3.0f,float max_size = 0.1f,float Button_size = 1.0f,float Rotate_speed = 1.0f,glm::vec3 Color = glm::vec3(1.0f,1.0f,1.0f)){
        // 在绘制粒子前开启混合
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // 标准透明混合
        float time_now = glfwGetTime();
        if(is_exploding){
            // 启动着色器
            const Shader* shader = ServiceLocator<ShaderManager>::get()->useShader("magic");
            ...
            shader->setUniform("uCenterVelocity",Velocity );
            shader->setUniform("Button_size", Button_size);
            shader->setUniform("Rotate_speed", Rotate_speed);
            shader->setUniform("Color_in",Color);
