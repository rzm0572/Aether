...
//      * 具体设计思路如下
//     */
//     Particle_Flareback_Missle(int particle_num,
//                       unsigned int random_seed,
//                         const std::string& texture_path,
//                         glm::vec3 origin_v, float radius_bias){
//         // 加载纹理
...
//         // 着色器
//         ServiceLocator<ShaderManager>::get()->registerShader("flareback_missle", getShaderPath("particles/flareback_missle.vert"), getShaderPath("particles/flareback_missle.frag"));
//         // 解绑
//         glBindVertexArray(0);
//     }
//     ~Particle_Flareback_Missle(){
//         glDeleteVertexArrays(1, &VAO);
//         glDeleteBuffers(1, &VBO);
//         glDeleteBuffers(1, &EBO);
//     }
...
//     */
//     void draw(glm::vec3 World_pos, glm::mat4 view, glm::mat4 proj, glm::vec3 camera_pos,
//                 float scale,float max_radius,float life_time,float max_size,
//                 glm::vec3 element_v){
//         // 在绘制粒子前开启混合
//         glEnable(GL_BLEND);
//         glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // 标准透明混合
...
//         if(is_exploding){
//             // 启动着色器
//             const Shader* shader = ServiceLocator<ShaderManager>::get()->useShader("flareback_missle");
...
//             shader->setUniform("Max_size", max_size);// 设定粒子最大尺寸
//             shader->setUniform("element_Velocity", element_v);// 设定粒子相对物体的速度（实际输入的就是物体的速度）
//             shader->setUniform("uMaxRadius_frac", max_radius/_radius_bias);// 设定尾焰最大半径
...
//     bool loaded_texture = 1;
//     bool is_exploding=false;// 是否正在爆炸
//     float _radius_bias=0.5f;// 尾迹半径，用于控制尾迹大小
// };


// // GPU_Ribbon 效果
// class Particle_Ribbon_GPU {
// public:
//     struct Particle {
//         glm::vec3 pos;         // 世界位置
//         float time;            // 发射时间
...
//     Particle_Ribbon_GPU(int max_particles,
//                     const std::string& texture_path)
//         : max_particles(max_particles){
...
//         ServiceLocator<ShaderManager>::get()->registerShader(
//             "ribbon_gpu", 
//             getShaderPath("particles/ribbon_gpu.vert"), 
//             getShaderPath("particles/ribbon_gpu.frag")
//         );
//     }
...
//         const Shader* shader = ServiceLocator<ShaderManager>::get()->useShader("ribbon_gpu");
//         shader->setUniform("uView", view);
//         shader->setUniform("uProjection", proj);
