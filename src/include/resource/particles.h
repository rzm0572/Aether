...
public:    
     ...
     * 具体设计思路如下
    */
    Particle_Ribbon(int particle_num,
                      unsigned int random_seed,
                        const std::string& texture_path,
                        glm::vec3 origin_v, float radius_bias){
        // 加载纹理
        ...
        */
        ...
        // 着色器
        ServiceLocator<ShaderManager>::get()->registerShader("ribbon", getShaderPath("particles/ribbon.vert"), getShaderPath("particles/ribbon.frag"));
        // 解绑
        glBindVertexArray(0);
    }
    ~Particle_Ribbon(){
