...
public:    
    ...
    void use(float dt,glm::vec3 pos,glm::vec3 Velocity,glm::vec3 up,glm::vec3 right,glm::vec3 forward,glm::quat rotation ,glm::vec3 target,glm::mat4 view,glm::mat4 projection,ThirdPersonCamera third_person_camera){
        flareback.draw(pos, view, projection, third_person_camera.getPosition(), 20.0f, 1.0f, 0.1f, forward);
        // == 根据输入发射武器或者根据计时器自动发射 ==
        bool Fire = false;
            ...
            if(glfwGetTime() - _last_time > 0.01f){// 间隔时间
                ...
                if(_input.getKeyPressed(InputKey::U)){
                    switch(weapon_set){
                        case 0:
                            // 空射导弹
                            weapon_kind = (weapon_kind + 1)%2;
                        break;
                        case 1:
                            // 机炮+魔法
                            weapon_kind = (weapon_kind + 1)%3;
                        break;
                        case 2:
                            // 火力支援
                            weapon_kind = (weapon_kind + 1)%2;
                ...
                case 0:
                    ...
                    if(weapon_kind == 0){
                        ...
                        if(slot != -1){
                            ...
                            // missle_flarebacks[slot] = Particle_Flareback(1000, 42, getAssetPath("textures/particle_generated.png"), glm::vec3(0.0f, 0.0f, 0.0f), 0.2f);
                            ...
                            missle_flarebacks.push_back(Particle_Flareback(1000, 42, getAssetPath("textures/particle_generated.png"), glm::vec3(0.0f, 0.0f, 0.0f), 0.2f));
                            missle_flarebacks.back().start_();
                        }
                        std::cout<<"fire missle "<<missles.size()<<std::endl;
                    }
                    else{
        ...
        for(int i=0;i<missles.size();i++){
            if(missle_is_active[i]){
                std::cout<<"draw missle "<<i<<std::endl;
                std::cout<<"missle_start_time[i]:"<<missle_start_time[i]<<" _last_time_boom:"<<_last_time_boom<<" glfwGetTime():"<<glfwGetTime()<<std::endl;
                if(missle_start_time[i] + _last_time_boom >= glfwGetTime()){
                    missles[i]->update(dt,2,target);
                    _renderer.submit_recursive(missles[i]);
                    missle_flarebacks[i].draw(missles[i]->getTransformComponent().getPosition(), view, projection, third_person_camera.getPosition(), 20.0f, 5.0f, 0.1f, missles[i]->physical_component().GetForward());
                }
                else{
